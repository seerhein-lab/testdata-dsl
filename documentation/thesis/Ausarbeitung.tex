...
% openany,
  ...
  11pt,
% draft,            %% Finden von überlangen Zeilen (dargestellt durch schwarze Blöcke)
]{book}             %% Dokumentklasse (alternativ: article, report, etc.)
       
%% \linebreak für Zeilenumbruch
...
\usepackage{xspace}                         %% nicht erzwungene Leerzeichen (für \newcommand nützlich}
\usepackage{todo}
...
% Einstellungen für Listings mit Eclipse Code Style.
\usepackage{color}
\usepackage{courier}
\definecolor{sh_comment}{rgb}{0.12, 0.38, 0.18 }     % adjusted, in Eclipse: {0.25, 0.42, 0.30 } = #3F6A4D
\definecolor{sh_keyword}{rgb}{0.37, 0.08, 0.25}      % #5F1441
\definecolor{sh_string}{rgb}{0.06, 0.10, 0.98}       % #101AF9% Farbe für Kommentare
...
%% durchnummeriert. Möchte man weitere Ebenen hinzufuegen, so
%% muss dies hier angegeben werden. Zählung beginnt bei 0.
\setcounter{tocdepth}{1} % im Inhaltsverzeichnis anzeigen
...
\newcommand{\thema}{Modellierung und Generierung von Testdaten für Datenbank-basierte Anwendungen}
\newcommand{\abgabedatum}{25. Oktober 2013}
\newcommand{\ort}{Konstanz}
\newcommand{\studGang}{Master Informatik}

\newcommand{\geboren}{22.12.1981}
\newcommand{\geborenIn}{Bad Saulgau}
\newcommand{\autor}{Nikolaus Moll}
\newcommand{\autorMatNr}{287336}
\newcommand{\autorStrasse}{Lorenz-Vogel-Weg 18}
\newcommand{\autorPLZ}{72505}
\newcommand{\autorOrt}{Krauchenwies}

\newcommand{\prueferA}{Jürgen Wäsch}
\newcommand{\prueferATitle}{Prof. Dr.-Ing.}
\newcommand{\prueferAInstanz}{Hochschule Konstanz Technik, Wirtschaft und Gestaltung}
\newcommand{\prueferAAdresse}{Brauneggerstraße 55}
\newcommand{\prueferAPLZ}{78462}
\newcommand{\prueferAOrt}{Konstanz}

\newcommand{\prueferB}{Thomas Fox}
\newcommand{\prueferBTitle}{Dr.}
\newcommand{\prueferBInstanz}{Seitenbau GmbH}
\newcommand{\prueferBAdresse}{Seilerstr. 7}
\newcommand{\prueferBPLZ}{78467}
\newcommand{\prueferBOrt}{Konstanz}
...
\begin{document}
  ...
  \include{formal/abstract}
  \newpage
  \thispagestyle{empty}~
  
  \include{formal/authorship}
  \newpage
  \thispagestyle{empty}~  
  
  %% Inhaltsverzeichnis erstellen
  \cleardoublepage
  \phantomsection % nötig damit das Inhaltsverzeichnis in selbigem korrekt verlinkt wird und keine Warnung geworfen wird
  ...
  \include{content/Modellierung}
	\include{content/ModellierungRealisierung}
	\include{content/GenerierenVonTestdaten}
  \include{content/Fazit}
  %\include{content/Vorlage}
  
  %\phantomsection % nötig damit das Tabellenverzeichnis korrekt verlinkt wird
  %\cleardoublepage
  %\printnomenclature
  ...
  \phantomsection  % nötig damit das Abbildungsverzeichnis korrekt verlinkt wird
  \cleardoublepage
  \listoffigures
  \addcontentsline{toc}{chapter}{Abbildungsverzeichnis}
  
  %% Listingverzeichnis generieren
  \phantomsection  % nötig damit das Listingverzeichnis korrekt verlinkt wird
  \cleardoublepage
  \lstlistoflistings
  ...
  \phantomsection  % nötig damit das Literaturverzeichnis korrekt verlinkt wird
  \cleardoublepage
  \printbibliography  
  ...
  \begin{appendix}
	\include{content/AnhangModel}
	\include{content/AnhangGenerierteDSL}
  \end{appendix}
