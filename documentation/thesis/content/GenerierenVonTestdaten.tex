...
\label{sec:generieren:algorithmus}
  ...
  \label{sec:generieren:algorithmus:pseudocode}
		...
		Anschließend wird über die Tabellenliste iteriert. Wurde eine Tabelle noch nicht behandelt, wird die
...
DO
  ...
  ELSE CALL GeneriereDatenFürsAssoziativeTabelle(T);
  END IF;
END FOR;
CALL "ErweitereGenerierteDatenzuKonsistenenDaten"()
\end{lstlisting}

		\subsubsection{GeneriereDatenFürNichtassoziativeTabelle}
...
\begin{lstlisting}[caption=GeneriereDatenFürNichtassoziativeTabelle, label=listing:GeneriereDatenFürNichtassoziativeTabelle]
GeneriereDatenFürNichtassoziativeTabelle(Tabelle T)
---------------------------------------------------
...
\end{lstlisting}
		...
		\subsubsection{GeneriereDatenFürKante}
...
\begin{lstlisting}[caption=GeneriereDatenFürKante, label=listing:GeneriereDatenFürKante]
GeneriereDatenFürKante(Kante K)   
------------------------------
...
\end{lstlisting}
		...
		\subsubsection{GeneriereEntitätenUndBeziehungen}
...
\begin{lstlisting}[caption=GeneriereEntitätenUndBeziehungen, label=listing:GeneriereEntitätenUndBeziehungen]
GeneriereEntitätenUndBeziehungen(K, s, z)
-----------------------------------------
...
THEN Berechne Entität e in Zieltabelle, die in keiner Beziehung zur Starttabelle stehen darf;
     CALL GeneriereEntitätenUndBeziehungen(k, 1, z);
...
THEN Berechne Entität e in Starttabelle, die in keiner Beziehung zur Zieltabelle stehen darf;
     CALL GeneriereEntitätenUndBeziehungen(k, s, 1);
...
CALL GeneriereAssoziativeEntitätenUndBeziehungen(T, min(LM), min(RM));
CALL GeneriereAssoziativeEntitätenUndBeziehungen(T, min(LM), max(RM));
CALL GeneriereAssoziativeEntitätenUndBeziehungen(T, max(LM), min(RM));
CALL GeneriereAssoziativeEntitätenUndBeziehungen(T, max(LM), max(RM));
LT := Ziel von Kante LK
RT := Ziel von Kante RK
...
THEN Markiere Tabelle LT als besucht;
     ...
     ELSE CALL GeneriereDatenFürsAssoziativeTabelle(LT);
     END IF;
END IF;
...
THEN Markiere Tabelle RT als besucht;
     ...
     ELSE CALL GeneriereDatenFürsAssoziativeTabelle(RT);
     END IF;
END IF;
\end{lstlisting}

		\subsubsection{GeneriereAssoziativeEntitätenUndBeziehungen}

\begin{lstlisting}[caption=GeneriereAssoziativeEntitätenUndBeziehungen, label=listing:GeneriereAssoziativeEntitätenUndBeziehungen]
GeneriereAssoziativeEntitätenUndBeziehungen(T, l, r)
----------------------------------------------------
LK := linke Kante der assoziativen Tabelle T;
RK := rechte Kante der assoziativen Tabelle T;
