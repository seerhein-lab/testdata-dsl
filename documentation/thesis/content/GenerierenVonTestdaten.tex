...
vom SUT verborgen werden, andererseits eigenen sie sich eher für Fixtures, die für einen einzelnen Unit-Test
verwendbar sind. Die Anonymisierung realer Daten stellt keine Daten-Generierung im eigentlichen Sinn dar und
...
Es gibt bereits eine Reihe von Werkzeugen zur Generierung von Zufallsdaten für Datenbanken. Inwieweit sich diese
für die Aufgabenstellung nutzen lassen, soll im Folgenden kurz analysiert werden.
  ...
  notwendige Fall abgedeckt sein. Die Menge der zu erzeugenden Testdaten lässt sich für jede Tabelle individuell konfigurieren.
  ...
  In \cite{Houkjaer:2006:SRD:1182635.1164254} wird ein Algorithmus zur Generierung von Testdaten vorgestellt, der
  ...
  Testdaten zu reduzieren. Eine Äquivalenzklasse fasst Eingangsdaten zusammen, für die das Verhalten des SUT dasselbe
	ist. Tests werden statt mit vielen Werten aus einer Klasse nur mit wenigen oder einem Wert durchgeführt 
	\cite[114]{BASISWISSEN_SOFTWAETEST}. Die Grenzwertanalyse stellt eine Ergänzung dar: Fehler treten häufig an Grenzfällen
	auf \cite[125]{BASISWISSEN_SOFTWAETEST}. In Hinblick auf zu generierende Beziehungen ist es eine Möglichkeit, mit den
	Grenzfällen die minimalen und die maximalen Beziehungsmöglichkeiten abzudecken.
...
beschrieben. Grundsätzlich können die beiden Typen A und B auch denselben Typen darstellen.

Ganz allgemein lassen sich alle binären Beziehungen als n..N:m..M-Beziehung ansehen. n und m stellen jeweils
untere Grenzen dar, N und M die oberen. Die grundlegende Generierungsstrategie sieht die Generierung der folgenden
...
die von den Grenzen der Multiplizitäten bestimmten Äquivalenzklassen abzudecken.
  ...
  keiner Entität in Beziehung stehen kann.
    ...
    Im Gegensatz zum vorherigen Beziehungstyp muss bei diesem eine Entität nicht zwingend in Beziehung mit einer anderen stehen.
    ...
    eine Entität ohne und eine Entität mit einer Beziehung (siehe Abbildung \ref{img:generierung:01to01}).
  ...
  Die allgemeinste Form einer Beziehung zwischen zwei Entitätstypen stellt eine n..N:m..M-Beziehung dar. Dabei handelt es
  ...
  eine oder beide oberen Grenzen 1 sind. Sind beide oberen Grenzen größer als 1, dann wird eine solche Beziehung
  ...
  einer 1..3:0..4-Beziehung dargestellt.
    ...
    dargestellten Entitäten und Beziehungen vorsehen.
...
Entitätstypen mit mehr als nur einem anderen Entitätstyp in Beziehung stehen und auch mit demselben Entitätstyp mehr als
nur ein Mal.

Bezüglich der Datengenerierung lassen sich hierbei zwei generelle Vorgehensweisen unterscheiden:
\begin{itemize}
...
Begriffe beschrieben und das Klassen-Diagramm des Modells vorgestellt, das der Daten-Generator verwendet.
  ...
  folgenden Abschnitt beschrieben wird.
    ...
    \subsubsection{Generiere Testdaten}
    
    Die Funktion \texttt{Generiere\_Testdaten} (siehe Listing \ref{listing:GeneriereTestdaten}) ist der Einstiegspunkt
    für den Algorithmus zur Generierung von Testdaten.
    ...
    die Constraints erfüllen. Gegebenenfalls werden hier Entitäten nachgeneriert.
  
\begin{lstlisting}[caption=Generiere Testdaten, label=listing:GeneriereTestdaten]
GeneriereTestdaten
------------------
...
\begin{lstlisting}[caption=Generiere Daten für nichtassoziative Tabelle, label=listing:GeneriereDatenFuerNichtassoziativeTabelle]
...
\end{lstlisting}
    ...
    Die Funktion zum Generieren von Daten für eine Kante setzt die in 
		Abschnitt~\ref{sec:generieren:beziehungen:nm} beschriebenen Schritte um (siehe Listing \ref{listing:GeneriereDatenFuerKante}). 
    Es werden alle vier Kombinationen aus Minimum und Maximum behandelt (Zeilen 6-9). Die eigentliche Generierung 
    ist in eine Hilfsfunktion ausgelagert, die mit allen Min-Max-Kombinationen aufgerufen wird.

\begin{lstlisting}[caption=Generiere Daten für Kante, label=listing:GeneriereDatenFuerKante]
...
\end{lstlisting}
    ...
    \begin{figure}[htbp]
      \centering
       \includegraphics[width=0.5\textwidth]{images/generierung/tausch-at.png}
...
\begin{lstlisting}[caption=Generiere assoziative Entitäten und Beziehungen, label=listing:GeneriereAssoziativeEntitaetenUndBeziehungen]
...
\end{lstlisting}
    ...
    Ungültig bedeutet, dass sie in Bezug auf einer oder mehreren Kanten nicht in ausreichend vielen
		Beziehungen steht.
    ...
    eine Entität nicht der Fall sein, wird eine entsprechende Beziehung hergestellt. Dies kann sich 
		auf bereits überprüfte Tabellen auswirken. Deshalb wird die Validierung erneut von Anfang an
		durchgeführt, wenn eine Entität nachgeneriert werden musste.
		
    Unter Umständen würde dieser Algorithmus nie terminieren, weshalb eine Abbruch-Bedingung sinnvoll ist.
		Diese wird nicht im Pseudocode (Listing \ref{listing:ErweitereGenerierteDatenZuKonsistentenDaten}) dargestellt.
		
		Die Implementierung bricht nach Erreichen eines Grenzwertes für die nachträgliche Generierung von Entitäten
		ab. Das Problem wird in Abschnitt~\ref{sec:generieren:offenepunkte:unerfuellbar} thematisiert.
    
\begin{lstlisting}[caption=Erweitere generierte Daten zu konsistenten Daten, label=listing:ErweitereGenerierteDatenZuKonsistentenDaten]
...
DO
  ...
  DO
    ...
    DO
      ...
      THEN Berechne Entität f in Zieltabelle der Kante K, f darf keiner Beziehung zur Tabelle T stehen;
    ...
    DO
      IF (e erfüllt Constraints für Kante K nicht)
      THEN Berechne Entität f in Starttabelle der Kante K, f darf in keiner Beziehung zur Tabelle T stehen;
           Stelle Beziehung zwischen Entität e und Entität f her;
           CALL Erweitere_Generierte_Daten_Zu_Konsistenten_Daten;
					 RETURN;
      END IF;
...
\end{lstlisting}
  ...
  Raum zugeordnet sein.  Ein Professor kann beliebig viele Prüfungen beaufsichtigen. Eine Prüfung benötigt
  ...
  Die generierten Daten für das vollständige fortlaufende Beispiel befinden sich in Anhang~\ref{chap:anhang:generiertedsl}.
    ...
    Da zwei Knoten dieselbe Anzahl eingehender Kanten haben, wird als Sekundärkriterium die Reihenfolge
    ...
    für PROFESSOR erzeugt. Die Zuordnung der Entitäten ist in Tabelle \ref{tab:generierung:raum_professor}
		dargestellt. RAUM\_1 und PROF\_1 haben keine Beziehung, RAUM\_3 hat eine Beziehung mit fünf Professoren.
    ...
    Der Algorithmus iteriert über die Kanten von PROFESSOR. Dabei werden zuerst ausgehende Kanten betrachtet,
		dann die eingehenden. Die Tabelle hat hat nur zwei eingehende, von denen eine bereits besucht wurde. Die
  ...
  % 5:3
    ...
    und fünf Entitäten aus PROFESSOR stehen mit genau einer Entität aus PRUEFUNG in Beziehung (1. und 2. Fall).
    ...
    \begin{figure}[htbp]
      \centering
       \includegraphics[width=0.5\textwidth]{images/generierung/example_step_pruefung.pdf}
    ...
    der Tabellen befinden sich auch keine unbesuchten Tabellen mehr, so dass nun der letzte
    ...
    diese Entitäten nur eine optionale Beziehung zu einer Entität aus RAUM haben, sind die
    nachgenerierten Entitäten gültig. Eine Erweiterung der Daten ist somit nicht notwendig
    und die Generierung abgeschlossen.


\section{Praktischer Einsatz und Evaluation}

Der praktische Nutzen des Algorithmus wird neben dem fortlaufenden Beispiel an einem praktischen
Projekt überprüft. Außerdem wird die Bedeutung der Starttabelle bei der Traversierung des Graphen 
beschrieben.

  \subsection{Praktisches Projekt}
	
	Der praktische Einsatz des Datengenerators soll auch an einem realen Projekt evaluiert werden.
	Es handelt sich dabei um ein kleines Projekt für die öffentliche Verwaltung. Das Datenbank-Modell
	umfasst zehn Tabellen mit im Mittel etwa acht Spalten.
	
	Die generierten Daten lassen sich in die Datenbank einspielen. Dafür sind allerdings für
	einige Spalten Projekt-spezifische Werte-Generatoren notwendig. Die Daten lassen sich für einfache
	Tests nutzen und können in der Anwendung selbst betrachtet werden.
	
	Es bietet sich an, die Bezeichner für die Referenzen nach der Generierung mit Hilfe von
	Refaktorisierungswerkzeugen anzupassen und den Referenzen sinnvolle Namen zu geben.
	
	Insgesamt stellen die generierten Daten einen guten Ausgangspunkt für Testdaten dar. Sie sind
	für diese Anwendung mit geringem Aufwand generierbar und für Tests nutzbar.

  \subsection{Einfluss der Tabellenreihenfolge}
  \label{sec:generieren:evaluation:tabellenreihenfolge}

  Die Art und Weise, wie Beziehungen generiert werden, hängt von der Reihenfolge ab, in der die Tabellen und Kanten behandelt
  ...
  Tabellen gleich viele eingehende Kanten haben können, muss das deterministische Verhalten durch weitere Sortierkriterien
  sichergestellt werden. Dies kann z.B. der Tabellenname sein, oder auch die Reihenfolge, in der die Tabellen definiert
  worden sind.
...
Die Zufallszahlengeneratoren der Werte-Generatoren werden über Seeds initialisiert. Damit derselbe Werte-Generator für verschiedene
Spalten standardmäßig unterschiedliche Werte erzeugt, wird das Standard-Spalten-Seed mit Hilfe des Tabellen- und des Spaltennamen
...
zwei Spalten mit identischen Zufallswerten zu erzwingen.
...
Tabellen leer bleiben. Das Modell wird deshalb für jede Tabelle um einen Wert erweitert, der die Mindestanzahl der zu generierenden Entitäten
...
Ein paar Probleme und Herausforderungen werden im Folgenden aufgezeigt.
  ...
  \begin{itemize}
    ...
    \item Werte, die sich aus Werten anderer Spalten \textbf{zusammensetzen}
  \end{itemize}
  ...
  Tabellen A, B und C. Jede Entität aus A steht genau mit einer Entität aus B und einer Entität
  ...
  benötigt eine Entität in C für eine Beziehung. Es kommt entweder die Entität aus C in Frage,
