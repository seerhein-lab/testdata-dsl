...
Änderungen an Schnittstellen, bei denen Abwärtskompatiblität verzichtet wird, betreffen vor allem die Builder-Klassen zum
Beschreiben des Datenbankmodells für den Generator.

	\subsection{Builder-Klassen für das Datenbankmodell}
	
	- Umstellung von überladenen Methoden auf flexibleres, erweiterbareres Fluent API
  ...
  \subsection{Architektur der generierten Klassen}
	...
	nutzt diese selbst. Auf diese Weise können neue Funktionen ohne Berücksichtigung der Fluent Builder API 
	hinzugefügt werden. Von Erweiterungen der Fluent Builder API profitiert die Table Builder API ebenfalls.
...
\end{verbatim}
	...
	\begin{itemize}
		\item \textbf{Code-Qualität}: Es gibt verschiedene Ansätze, Klassen um neue Funktionen zu erweitern oder
		  ihr Verhalten zu ändern. Egal ob auf Vererbung oder Delegation gesetzt wird, werden neue Datentypen benötigt.
			
		  Soll die Schicht der Fluent Builder API nicht verändert werden, stellt Vererbung keine Option dar, wenn
			Klassen von der Fluent-Builder-API-Schicht selbst instantiiert werden. Delegierende Klassen bestehen aus
			einer Vielzahl von Methoden, die nichts anderes machen, als die Aufgabe dem Delegat zu übergeben.
