...
\label{sec:modellierung:generatormodell}
	...
	Die neuen Builder-Klassen decken den Funktionsumfang der alten API ab. So werden Flags für Spalten nicht mehr über 
	ein \texttt{EnumSet} festgelegt, sondern über Methoden für die vordefinierten Flags. In Abschnitt 
	\ref{sec:modellierung:generatormodell:flags}  wird weiter auf das Thema Flag eingegangen. Darüber hinaus bieten die
	...
	\begin{itemize}
		\item \textbf{Identifkator-Spalte}: Als Identifikator-Spalte wird die Spalte bezeichnet, die einen für die Zeile einmaligen
	...
	von 9 addColumn-Varianten werden hier nur 2 genutzt!
...
Tabelle abhängig. Der IDE-Support wird über die neuen DataSet-Builder-Klassen realisiert.
  ...
  Der Großteil des IDE-Supports wird über Adapter-Klassen für die bisherigen Tabellen realisiert. Zu jeder Tabellen-Klasse wird
	...
	IDE-Support bzgl. der Tabellennamen sichergestellt.

	\begin{figure}[H]
		\centering
		 \includegraphics[width=0.75\textwidth]{images/realisierung/builderarchitecture.pdf}
...
% http://martinfowler.com/eaaCatalog/registry.html
	...
	\begin{enumerate}
		\item \textbf{Tool-Support}: Elemente der Tabellen-Header-Zeile, Auto-Vervollständigung, Liste von nutzbaren Elementen
		
		\item \textbf{Operationen für den Parser}: Operationen auf den ColumnBindings vereinfachen den Tabellen-Parser. Der
		  Tabellen-Parser setzt die Werte auf den RowBuildern. Die Setter-Namen hängen aber vom Namen der Spalte ab. Anstelle
			eines generischen Ansatzes über Reflection, wird ein allgemeiner Setter auf der ColumnnBinding-Klasse verwendet,
			und in den jeweiligen zu den Spalten zugehörigen ColumnBindings wird auf den eigentlichen Setter auf dem RowBuilder
			delegiert.
			
			Außerdem kann der Parser auch Informationen zu der Spalte abfragen, z.B. ob es sich um ein Feld mit einmaligen Werten
			handelt, die zur Identifizierung genutzt werden können. Für solche Spalten gibt es auch eine Such-Methode auf dem
			ColumnBinding, die den zugehörigen RowBuilder zurückliefern, falls es einen gibt.
	\end{enumerate}
	...
	In STU ist eine Referenz eine Art Stellvertreter für eine Entität (Tabellenzeile). Die Referenz kann bei der Modellierung 
	anstelle konkreter Werte (wie Primärschlüssel) verwendet werden, oder bei Such-Anfragen anstelle von konkreten Werten
...
dataSet.beaufsichtigtTable.deleteAllAssociations(HAASE);
  \end{lstlisting}
	...
	\begin{itemize}
		...
		\item \textbf{Code-Qualität}: Nach Robert C. Martin soll eine Funktion (bzw. Methode) genau eine Aufgabe erledigen. Wenn
