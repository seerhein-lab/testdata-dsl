...
Mit Hilfe dieser Analyse wird die finale Modellierungssprache (DSL) definiert. Bei den Bewertungskriterien wird auf den
  ...
  Die in Listing \ref{listing:dslentwurf1} gezeigte DSL kommt ohne manuell vergebene Schlüsselwerte (Primary-Keys)
	...
	\begin{itemize}
		...
		\item \textbf{Zeichen pro Zeile}: Der Entwurf ist für die Bildschirmdarstellung gut geeignet, die Beispieldaten
		  erreichten höchstens 50 Zeichen pro Zeile.
...
...
  ...
  m:n-Beziehung zu Prüfungen.
	...
	\begin{itemize}
		...
		\item \textbf{Zeilen}: Der größte Vorteil dieser Darstellung ist, dass nicht jeder Wert, sondern dass
		...
		\item \textbf{Zeichen pro Zeile}: 
		  Die Beispieldaten erreichen eine maximale Breite von bis zu 144 Zeichen bei einer Tabelle mit 10 Spalten.
			...
			horizontal. Optionale bzw. kaum genutzte Spalten erhalten bei dieser Darstellung unter Umständen viel 
			Platz. 
		
		\item \textbf{Redundanz}: Wie die beiden bisherigen Entwürfe tritt hier keine erkennbare Daten-Redundanz auf.
...
direkt in vollständig komplatbilen Java-Bytecode übersetzt wird und damit auch in einer Java Virtual Machine
...
den Zugriff von nicht definierten Klassenelementen zu reagieren.
  ...
  \nomenclature{AST}{Abstract Syntax Tree}
    ...
    Der Ansatz ist, Operator-Überladen für das Parsen zu verwenden. Der Operator \texttt{or} ist ein binärer
...
class TableParser {
  ...
  def parseTableClosure(Closure tableData){
    ...
    (Zeile 22). Dieser Delegat wird bei der Auflösung von Variablen- und Methoden-Bezeichern verwendet, d.h. es können
		innerhalb des Closures alle Elemente des Delegaten genau so verwendet werden, als wären sie im Closure selbst definiert.
		In Zeile 23 wird festgelegt, dass bei der Auflösung zuerst im Delegaten gesucht wird.
...
class TableParser {
  ...
  def parseTableClosure(Closure tableData){
    ...
    Die statischen \texttt{or}-Methoden haben keinen Zugriff auf Instanz-Variablen der Klasse \texttt{TableParser}. 
		Ihre Zwischenergebnisse, also die geparsten Spaltenwerte, können nur in statische Elementen abgelegt werden.
    ...
    werden, um einem Datensatz keinen Bezeichner für Referenzen zu zu weisen. 
...
Entsprechend der Kriterien stellt sich die Bewertung für die Sprache wie folgt dar:
	...
	\begin{itemize}
		...
		\item \textbf{Modifizierbarkeit}: Die Daten lassen sich relativ leicht an neue Datenbank-Schemata anpassen
		  und die Daten selbst sich leicht ändern. 
