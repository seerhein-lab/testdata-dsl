...
mit Java-Anwendungen. 
	...
	\nomenclature{AST}{Abstract Syntax Tree}
		...
		enthält dafür die Methode \texttt{parseTableClosure}.
  ...
  def parseTableClosure(Closure tableData){
    use(TableParser) {
      tableData()
		...
		Doch auch solche Operationen lassen sich überschreiben, wenn für die Datentypen passende Kategoriemethoden
		definiert werden.
...
class TableParser {
  ...
  def parseTableClosure(Closure tableData){
    use(TableParser) {
      tableData.delegate = this		// Change context
      ...
      tableData()
		...
		Realisierung:
		- da statische Operatoren Speichern der Ergebnisse in ThreadLocal-Variable \cite{JAVA_CONCURRENCY_IN_PRACTICE}
		- Kontext des Closures geändert für getProperty
