...
class HochschuleDataSet extends HochschuleBuilder
{
  ...
  def relations() {
	...
	Beziehungen modelliert. Assoziativer Beziehungen können mit Hilfe der DSL nicht nur auf Datenbank-Ebene,
...
class HochschuleDataSet extends HochschuleBuilder
{
  ...
  def tables() {
	...
	\subsection{Nutzung des DataSets in einem JUnit-Test}
	...
	zeigt Listing \ref{listing:junittest}. Beim getesteten System (System under test), repräsentiert von der Variable
	\texttt{sut} in Zeile  21, handelt es sich um einen Spring-Service \todo{Quelle Spring Service}. 
	
	Das in einem Test verwendete DataSet kann als Klasse über die Annotation \texttt{DatabaseSetup} konfiguriert werden
	(Zeile 27). Sie sorgt dafür, dass die angegebene DataSet-Klasse instantiiert und der Variable zugewiesen wird, die 
	mit der Annotation \texttt{InjectDataSet} markiert wurrde (Zeilen 23 und 24). Außerdem wird dieses DataSet auch bei
	...
	Entität übergeben werden muss (Zeile 39), wird in den Zeilen 30 bis 36 eine entsprechende Instanz erstellt und
	konfiguriert. 
...
public class HochschuleDataSetDatabaseTest
{
  ...
  public void removeStudent() throws Exception {
	...
	In den Zeilen 42 und 43 werden die erwarteten Änderungen im DataSet ebenfalls durchgeführt, um in Zeile 45 die
	Datenbank gegen das DataSet zu vergleichen.
	
	Die neue DSL kann in Groovy-basierten Tests verwendet. Listing \ref{listing:junittest:groovy} zeigt beispielhaft 
	eine entsprechende Test-Methode. In diesem Test wird eine neue Lehrveranstaltung erstellt und einem Professor
	zugeordnet.
