...
Als eigenständiges Projekt kann in \textit{STU} auf Abwärtskompatibilität verzichtet werden. 
...
\begin{itemize}
	\item Das \textbf{Fluent Builder API} ist ein \textbf{Java}-basiertes API. Es basiert auf \textit{SB Testing DB} und ist
	  zu diesem abwärtskompatibel. Der Name spiegelt wieder, dass ein Java Fluent API verwendet wird (siehe auch Abschnitt 
		\ref{sec:grundlagen:sbtestingdb}). Ein solches API wird auch als interne DSL bezeichnet.
...
...
  ...
  \subsection{Gewählter DSL-Entwurf}
	...
	denkbar, die sich an die Modellierung der Beziehungen in Listing \ref{listing:dslentwurf1} orientiert.
...
grundsätzlich in Java realisiert werden, doch die Möglichkeiten diesbezüglich sind relativ eingeschränkt 
...
def dataset = [
  ...
  ...
    ...
    Insgesamt muss viel Aufwand betrieben werden, um den AST zu analysieren. Dabei wirft die IDE-Unterstützung noch
		einige Fragezeichen auf: Eine IDE-Unterstützung für Labels (Auto-Vervollständigung, Vorschläge) ist mit einfachen
		Mitteln kaum realisierbar.
	...
	ausdrucksstarken und damit wartungsunfreundlicheren Codes. Als Konsequenz wird die einfachere
...
\label{sec:modellierung:generatormodell}
	...
	umgesetzt. So enthält das alte API neun Methoden zum Hinzufügen einer Spalte in einer Tabelle
	...
	\begin{itemize}
		\item \textbf{Identifier}: Als Identifikator-Spalte wird die Spalte bezeichnet, die einen für die Zeile einmaligen
		  Wert enthält, über die eine Zeile zweifelsfrei identifiziert werden kann. Ein Primärschlüssel stellt eine solche
			Spalte dar, ist jedoch keine Voraussetzung für dieses Flag. Das Flag wird über die Methode \texttt{identifier()}
			aktiviert. Dabei wird das Flag \texttt{Immutable} implizit aktiviert.
...
WAESCH.beaufsichtigt(P_VSYS)
	...
	vorliegt (Zeile 6). Da dieses Feld durch \textit{Depenency Injection} (\todo{Literatur dependency injection}) erst
...
dataSet.table_Professor.findWhere.vorname(HAASE);
	  ...
	  \item \textbf{getWhere}: Analog zu \texttt{findWhere} gibt es die Möglichkeit, einzelne Zeilen über \texttt{getWhere}
		  abzufragen. Der einzige Unterschied zwischen beiden ist, dass \texttt{getWhere} das Ergebnis in Form eines 
			...
			erwartet wird, kann \texttt{getWhere} anstelle von \texttt{findWhere} genutzt werden.
  ...
  \subsection{Nicht umgesetzt}
	...
	warum diese Funktion nicht in \textit{STU} implementiert ist.
		...
		\begin{itemize}
  	    ...
  	    \begin{lstlisting}[caption=Reflexiv, label=listing:reflexiv]
