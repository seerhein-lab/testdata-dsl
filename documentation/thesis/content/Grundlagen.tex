...
\begin{itemize}
	\item \textbf{Plattform}: Die DSL soll sich in der bisherigen Werkzeug-Kette von Seitenbau nutzen lassen. Das heißt,
		die DSL muss sich in Java integrieren lassen.
	\item \textbf{Beziehungen}: Die Modellierung von Beziehungen soll vereinfacht werden.
	\item \textbf{Typ-Sicherheit}: Beziehungen und Daten sollen Typ-sicher modelliert werden können. Spätestens bei der
		Ausführung der Tests müssen ungültige Beziehungen und falsche Typen erkannt werden.
\end{itemize}
...
dataSet.addTable(lehrveranstaltung);
  \end{lstlisting}
	...
	aufzufangen.
	Ein Generator erzeugt aus Meta-Informationen zu den Tabellen eine einfache Java-DSL. Über diese DSL können
