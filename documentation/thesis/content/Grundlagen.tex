...
\begin{itemize}
	\item \textbf{Standard Fixture}: Ein Test Fixture wird als Standard Fixture bezeichnet, wenn es für alle bzw. fast alle 
	  Tests verwendet werden kann. Ein Standard Feature reduziert nicht nur den Aufwand zum Entwerfen von Testdaten für die
		einzelnen Tests, sondern verhindert darüber hinaus, dass der Test-Ingenieur  sich bei verschiedenen Tests immer wieder
...
\begin{itemize}
	...
	\item \textbf{Table}: Für jede Tabelle wird eine entsprechende Klasse generiert. Der Klassenname setzt sich aus dem Namen
	  der Tabelle und dem Suffix "`Table"' zusammen. Die Klasse stellt Methoden zum Modellieren der Daten und für den Zugriff
		...
		Einzelne Tabellenzeilen werden von einer passenden RowBuilder-Klasse repräsentiert.
...
Ein vereinfachtes Klassendiagramm der DataSet-Klassen ist in Abbildung \ref{img:sb_testing_classes} dargestellt. Die Vereinfachung
besteht darin, dass im Diagramm alle \texttt{Table}-Klassen zusammengefasst werden, obwohl diese unterschiedlichen Typs sind. Außerdem
entsprechen die Klassennamen, bis auf die Klasse \texttt{FindWhere}, nicht den tatsächlichen Bezeichnern.
