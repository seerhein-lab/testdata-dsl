...
\todo{Modellgetriebene Software-Entwicklung ergänzen: Beispiele, DSL, intern vs extern}
...
\end{itemize}
	...
	hinter diesem Muster ist es, auf die Datenbank direkt zu zu greifen am zu testenden System vorbei -- quasi in Form
	einer Hintertür. Auf diese Weise wirken sich Fehler im zu testenden System nicht auf den Zugriff der Datenbank aus.
	Das Muster besteht aus vier Schritten. 
	...
	Abbildung \ref{img:backdoormanipulation} stellt Back Door Manipulation grafisch dar.
	...
	\todo{Datenbank-Tests: Layer Test erklären}
...
kann mit beliebig vielen, aber mindestens einer Entität aus \textit{Tabelle 2} in Beziehung stehen. Eine Entität aus
\textit{Tabelle 2} muss mit genau einer Entität aus \textit{Tabelle 1} in Beziehung stehen.
  ...
  \subsection{ER-Diagramme}
	...
	genutzt wird wird. Dieser Stil erweitert das dem UML-konform das Klassendiagramm.
	...
	\begin{figure}[H]
		...
		\caption{Stil relatianaler Datenbank-Diagramme nach Ambler}\label{img:ambler_table}
	\end{figure}
...
Erweiterung für JUnit dar und stellt Funktionen speziell für den Test von Datenbank-Anwendung zur Verfügung
...
Ein Ziel von \textit{STU} ist, Datenbank-Tests weiter zu vereinfachen und bietet die Möglichkeit, DataSets über ein eigenes API
...
Abbildung \ref{img:sbtestingmodel} stellt grafisch dar, wie aus einem Datenbank-Modell das Fluent API erzeugt wird.
Ausgangspunkt ist ein relationales Datenbankmodell. Dieses Modell muss in ein für den Generator interpretierbares
Modell, das STU-Modell, transformiert werden. Dies kann automatisch (z.B. wenn das Modell in Form eines
\textit{Apache-Torque}-Modell vorliegt) oder manuell geschehen. Das STU-Modell enthält Informationen zu Tabellennamen
...
Das STU-Modell enthält keine Datenbank-Constraints. Eine Abbildung dieser würde keine wesentliche Vorteile bringen. Das API
