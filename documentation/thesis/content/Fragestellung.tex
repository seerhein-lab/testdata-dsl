...
\begin{itemize}
	\item \textbf{Kompatibilität}: Die Generierung der Testdaten soll nicht nur bei Nutzung der neuen Modellierungssprache
	  verwendet werden können. Der Testdaten-Generator soll auch DataSets auf Basis der bisherigen STU-Builder
		erstellen können.
\end{itemize}
...
\section{Kriterien für Sprachbewertung}
\label{sec:fragestellung:sprachkriterien}

\begin{itemize}

	\item Zeilen

	\item Zeichenverhältnis
...
dataSet.addTable(lehrveranstaltung);
  ...
  Art nur wenig Vorteile gegenüber den XML-DataSets.
	...
	Datenbank-Modell-spezifischen API. Ein Generator erzeugt aus dem Datenbank-Modell ein Java Fluent Builder
	API, das die Modellierung von Daten für genau dieses Datenbank-Modell erlaubt (siehe auch \ref{sec:grundlagen:stu}). 
