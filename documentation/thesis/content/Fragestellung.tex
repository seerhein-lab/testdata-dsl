...
Um den Umgang mit DbUnit-DataSets zu vereinfachen, hat SEITENBAU die Bibliothek \textit{SB Testing DB}
entwickelt. \textit{SB Testing DB} erweitert DbUnit um zusätzliche Funktionen, löst aber nicht alle
Schwierigkeiten bei der Modellierung von Test-Daten. Eine genauere
Betrachtung der Vor- und Nachteile verschiedener Modellierungsvarianten in Zusammenhang mit DbUnit
wird in Abschnitt \ref{sec:fragestellung:modellierung} durchgeführt. 
...
Modellierungsvarianten.
	...
	Abbildung \ref{img:example_er} zeigt das Beispiel grafisch in Form eines ER-Diagramms (Beschreibung siehe Abschnitt 
	\ref{sec:grundlagen:konventionen:erdiagramme}). Den verschiedenen Entitätstypen
	...
	Das entsprechende relationale Datenbank-Diagramm (siehe Abschnitt \ref{sec:grundlagen:konventionen:relationalediagramme}) 
	wird in Abbildung \ref{img:example_relational} dargestellt. 
...
dataSet.addTable(lehrveranstaltung);
  \end{lstlisting}
	...
	Diese Variante löst allerdings nicht alle Probleme: So müssen immer noch Meta-Informationen über die Tabellen
  ...
  Art nur wenig Vorteile gegenüber den XML-DataSets.
	...
	Ein Generator erzeugt aus einem Datenbank-Modell ein Java Fluent Builder API, das die Modellierung von
	Daten für genau dieses Datenbank-Modell erlaubt. \textit{SB Testing DB} führt eine eigene DataSet-Klasse ein,
	über die die Daten modelliert werden. Diese DataSet-Klasse kann bei Bedarf von den aktuellen Daten ein
	DbUnit-DataSet erzeugen. Auf diese Weise können DataSets aus \textit{SB Testing DB} einfacher und umfangreicher
	als DbUnit-DataSets modifiziert werden, wie z.B. das Löschen von Zeilen.
	
	Auf diese Weise können mit \textit{SB Testing DB} verhältnismäßig einfach Varianten eines DbUnit-DataSets
	erzeugt werden, z.B. ein DataSet mit dem Ausgangszustand und ein DataSet mit dem erwarten Zustand am Ende des Tests.
	
	Die Java-DSL sorgt für statische Typsicherheit, so dass IDEs wie Eclipse fehlerhafte Typen bereits während der
