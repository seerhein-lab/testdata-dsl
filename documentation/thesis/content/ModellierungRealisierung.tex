...
Tabelle abhängig ist. Wie der IDE-Support über die neuen DataSet-Builder-Klassen realisiert wird, wird im folgenden beschrieben.
...
ergänzt werden. Eine Fassade stellt eine Schnittstelle auf höherer Abstraktionsebene dar, um das System einfach zu verwenden
...
\begin{itemize}
  ...
  \item \textbf{Einheitlicher Funktionsumfang}: Das Table Builder API und das Fluent Builder API sollen -- so weit
	  möglich und sinnvoll -- den selben Funktionsumfang bieten. Erweiterungen für das Fluent Builder API stehen
...
\label{sec:modellierung:generatormodell}
  ...
  \textit{STU} sieht verschiedene Eigenschaften für Spalten vor, sogenannte Flags. Die Flags sind bislang in einem \textit{Enum}
...
2  | 123456         | "Nikolaus"       
  ...
  der Ausgangspunkt wird durch die Ebene bestimmt. Diese Bezeichner werden für die Methoden zur Modellierung
  ...
  \ref{listing:model:builder:new} die neuen Builder veranschaulicht.
...
Table person = table("person")
  ...
  .build();
        \end{lstlisting} 
			  ...
			  Das selbe Modell lässt sich -- aus Sicht von \textit{STU} -- etwas typsicherer umsetzen.
...
associativeTable("parcipations")
  ...
  .build();
      ...
      \item \textbf{Ternäre und andere höhergradigen Beziehungen}
			  \textit{STU} sieht keine spezielle Unterstützung für ternäre oder andere höhergradigen Beziehungen vor.
        ...
        \begin{lstlisting}[caption=Höhergradige Beziehungen mit zusätzlicer Tabelle, label=listing:hoehergradig]
